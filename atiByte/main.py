...
void main()
{
    ...
    v_texture = a_texture;
    normalVector = mat3(model) * a_normal;
...
void main()
{
    ...
    diffusionLevel = max(0.0,dot(norm,lightDir));
    lightLevel = lightCol*diffusionLevel + ambient;
    out_color = vec4(lightLevel,1.0) * texture(s_texture, v_texture);
}
"""
...
cube_indices, cube_buffer = ObjLoader.load_model("cube.obj", False)
monkey_indices, monkey_buffer = ObjLoader.load_model("monkey.obj")
floor_indices, floor_buffer = ObjLoader.load_model("floor.obj")

shader = compileProgram(compileShader(vertex_src, GL_VERTEX_SHADER), compileShader(fragment_src, GL_FRAGMENT_SHADER))
glUseProgram(shader)
glUniformMatrix4fv(glGetUniformLocation(shader,'ambient'), 1, GL_FALSE, pyrr.Vector3([0.05,0.05,0.05]))
glUniformMatrix4fv(glGetUniformLocation(shader,'lightCol'), 1, GL_FALSE, pyrr.Vector3([1,1,1]))
glUniformMatrix4fv(glGetUniformLocation(shader,'lightPos'), 1, GL_FALSE, pyrr.Vector3([5,5,5]))
...
load_texture_pygame("cube.jpg", textures[0])
load_texture_pygame("monkey.jpg", textures[1])
load_texture_pygame("floor.jpg", textures[2])
